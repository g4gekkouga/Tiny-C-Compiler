...
file: parser code in bison, .y file 
*/
...
%token AUTO BREAK CASE CHAR CONTINUE DEFAULT DO DOUBLE ELSE 
%token FLOAT ENUM EXTERN CONST FOR GOTO IF INLINE INT LONG RETURN SHORT SIGNED SIZEOF STATIC STRUCT SWITCH MATRIX REGISTER RESTRICT
%token TYPEDEF UNION_ UNSIGNED VOID VOLATILE WHILE _BOOL _COMPLEX _IMAGINARY
%token PTR_OP INCREMENT DECREMENT LEFT_OP RIGHT_OP LE_OP GE_OP EQ_OP NE_OP TRANSPOSE
%token AND_OP OR_OP ELLIPSIS MUL_ASSIGN DIV_ASSIGN MOD_ASSIGN ADD_ASSIGN SUB_ASSIGN
%token LEFT_ASSIGN RIGHT_ASSIGN AND_ASSIGN XOR_ASSIGN OR_ASSIGN SINGLE_LINE_COMMENT MULTI_LINE_COMMENT
