...
void quad_array::emit2(opcode opc,string arg1, string arg2, string result)
{
	if(result.size()==0)
	{
...
{ 
    ...
    { 
        ...
        quadSort(arr, low, pi - 1); 
        quadSort(arr, pi + 1, high); 
    } 
} 
...
int symtab__lookmax(int a, int b) 
{ 
    ...
    if (a > b) 
        return symtab__lookmax(a-b, b); 
    return symtab__lookmax(a, b-a); 
}
...
symdata* symtab::lookup(string n,bool search){
	...
	for (int j = 0; j < i; ++j)
	{
		if(symbol_tab[j]->name == n && symbol_tab[j]->ispresent)
			return symbol_tab[j];
	}
...
symdata* symtab::lookup_2(string n)
{
	...
	for(i=0;i<global_ST->symbol_tab.size();i++)
	{
		//printf("Flag1\n");
		if(global_ST->symbol_tab[i]->name == n)
		{
			return global_ST->symbol_tab[i];
		}
	}
	symdata *temp_o=new symdata(n);
	temp_o->i_val.int_val=0;
		...
		symdata * t = symbol_tab[i];
        	...
        	if(t->isInitialized){
        		switch((t->tp_n)->basetp){
        			...
        			case tp_double: printf("%.3lf\t",(t->i_val).double_val);break;
...
int main(int argc, char* argv[]){
	...
	char str[20];
	sprintf(str,"cc -S %s",argv[1]);
	system(str);
	*/
	
	symdata *temp_printi=new symdata("printInt");
	temp_printi->tp_n=new node_type(tp_int);
	temp_printi->var_type="func";
	temp_printi->nest_tab=global_ST;
	global_ST->symbol_tab.push_back(temp_printi);
	
	symdata *temp_readi=new symdata("readInt");
	temp_readi->tp_n=new node_type(tp_int);
	temp_readi->var_type="func";
	temp_readi->nest_tab=global_ST;
	global_ST->symbol_tab.push_back(temp_readi);

	symdata *temp_printf=new symdata("printFlt");
	temp_printf->tp_n=new node_type(tp_double);
	temp_printf->var_type="func";
	temp_printf->nest_tab=global_ST;
	global_ST->symbol_tab.push_back(temp_printf);
	
	symdata *temp_readf=new symdata("readFlt");
	temp_readf->tp_n=new node_type(tp_double);
	temp_readf->var_type="func";
	temp_readf->nest_tab=global_ST;
	global_ST->symbol_tab.push_back(temp_readf);
	
	symdata *temp_prints=new symdata("printStr");
	temp_prints->tp_n=new node_type(tp_int);
	temp_prints->var_type="func";
	temp_prints->nest_tab=global_ST;
	global_ST->symbol_tab.push_back(temp_prints);
	...
	do{
		...
		((global_ST->symbol_tab[i])->nest_tab)->print(); int j = incr(i);
		++i; int k = list_fa(i);
		k = symtab__lookmax(k, j);
	...
	set<string>setty_1;
	global_ST->mark_labels();
	global_ST->global_variables(fp);
	setty_1.insert("Global");
	int count_l=0;
	for (int i = 0; i < global_ST->symbol_tab.size(); ++i)
	{
		int j = incr(i);
		int k = list_fa(i);
		k = symtab__lookmax(k, j);
		if(((global_ST->symbol_tab[i])->nest_tab)!=NULL)
		{
			if(setty_1.find(((global_ST->symbol_tab[i])->nest_tab)->name) == setty_1.end())
			{
				global_ST->symbol_tab[i]->nest_tab->assign_offset();
				global_ST->symbol_tab[i]->nest_tab->print();
				global_ST->symbol_tab[i]->nest_tab->function_prologue(fp,count_l);
				global_ST->symbol_tab[i]->nest_tab->function_restore(fp);
				global_ST->symbol_tab[i]->nest_tab->gen_internal_code(fp,count_l);
				setty_1.insert(((global_ST->symbol_tab[i])->nest_tab)->name);
				global_ST->symbol_tab[i]->nest_tab->function_epilogue(fp,count_l,count_l);
				count_l++;
			}
		}
	}
