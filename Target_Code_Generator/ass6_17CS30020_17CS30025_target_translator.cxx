...
void symtab::rev_srt(int sym_arr[], int n)  
{  
    int i, j;  
    for (i = 0; i < n-1; i++)      
    ...
    for (j = 0; j < n-i-1; j++)  
        if (sym_arr[j] < sym_arr[j+1])  
            symtab::swap_reg(&sym_arr[j], &sym_arr[j+1]);  
}
...
void symtab::swap_reg(int *xp, int *yp)  
{  
    int tp_double = *xp;  
    *xp = *yp;  
    *yp = tp_double;  
...
void symtab::srt(int sym_arr[], int n)  
{  
    int i, j;  
    for (i = 0; i < n-1; i++)      
...
int symdata::minDist(int arr[], int n, int x, int y) 
{ 
   int i, j; 
   int min_dist = 100000; 
   for (i = 0; i < n; i++) 
...
void symtab::Max_Sq_Quad(bool M[6][5])  
{
	int R = 6;
	int C = 5;  
    int i,j;  
